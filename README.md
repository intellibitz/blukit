...
- **Unified Crowd Radar**: A high-fidelity, nested spatial paradigm. Each Crowd context in the ticker features its own **interactive Mini Radar** anchored by the Owner, with other users orbital around them. For the default crowd, the user's own **Identity Anchor** serves as the primary row icon, reinforcing collective ownership. The background remains as a pure atmospheric **Spectral Field** for spatial awareness.
- **Resonance Summaries**: A "Summary-First" visual paradigm. Headers display high-level context (Title, Member Count, "ENTER" affordance) and generic type labels (e.g., `EVENT` or `PRIVATE CHAIN`). The default crowd header features a prominent **interactive Persona Signature** of the user.
