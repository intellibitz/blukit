...
class NearbyP2PController(
    ...
    private val payloadCallback = object : PayloadCallback() {
        override fun onPayloadReceived(endpointId: String, payload: Payload) {
            payload.asBytes()?.let { bytes ->
                ...
                val bluetoothPayload = Json.decodeFromString<BluetoothPayload>(payloadJson)
        ...
        connectionsClient.startDiscovery(serviceId, object : EndpointDiscoveryCallback() {
            override fun onEndpointFound(endpointId: String, info: DiscoveredEndpointInfo) {
                _scannedDevices.update { devices ->
                    val newDevice = BluetoothDeviceDomain(info.endpointName, endpointId)
    ...
    override fun connectToDevice(device: BluetoothDeviceDomain): SharedFlow<ConnectionResult> {
        ...
        return MutableSharedFlow() // This is a simplified implementation for now
    }

    override suspend fun sendMessage(content: String, receiverId: String?): BluetoothPayload? {
        val nickname = repository.getNickname() ?: "Anonymous"
        val senderId = repository.getDeviceId()
        val payloadObj = BluetoothPayload(
            messageId = UUID.randomUUID().toString(),
            senderId = senderId,
            senderName = nickname,
            content = content,
            timestamp = System.currentTimeMillis(),
            receiverId = receiverId // I need to add receiverId to BluetoothPayload
        )
        ...
        if (receiverId != null) {
            ...
            // Broadcast to all active connections (Stadium Lobby)
