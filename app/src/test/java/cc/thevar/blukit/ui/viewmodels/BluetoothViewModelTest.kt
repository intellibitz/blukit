...
class BluetoothViewModelTest {
    ...
    fun setUp() {
        ...
        viewModel = BluetoothViewModel(p2pController, radioStateManager, repository, permissionManager, vibeStore, connectivityUseCase)
    }
    ...
    fun `test Harmony reflects The Vibes accurately`() = runTest {
        viewModel.state.test(timeout = 10.seconds) {
            ...
            assertEquals("Harmony should start cold", false, initial.harmony.isBluetoothEnabled)

            // Awaken The Vibes
            harmonyFlow.value = RadioStates(isBluetoothEnabled = true, isLocationEnabled = true, isWifiEnabled = true)
            
            val active = awaitItem()
            assertTrue("The Vibes should be alive in Harmony", active.harmony.isBluetoothEnabled)
            assertTrue("The Air should be clear in Harmony", active.harmony.isLocationEnabled)
    ...
    fun `test Awakening The Vibes triggers discovery`() {
        ...
        viewModel.startScan()
        // verify { p2pController.startDiscovery() } // This logic is now in UseCase, which is mocked here.
        // verify { p2pController.startAdvertising() }
        verify { radioStateManager.triggerRefresh() }
    }

    @Test
    fun `test Stilling The Vibes stops discovery`() {
        viewModel.stopScan()
        verify { connectivityUseCase.clearManualStatus() }
    }
    ...
    fun `test disturbed Air reflects in The Vibes`() = runTest {
        viewModel.state.test(timeout = 10.seconds) {
            ...
            assertEquals("The Vibes must reflect the disturbed Air", "The Air is Disturbed", disturbed.activity.uiError?.message)
    ...
    fun `test selection management`() = runTest {
        viewModel.state.test {
            ...
            assertTrue(awaitItem().crowd.selectedDevices.contains("device-1"))
            
            viewModel.toggleDeviceSelection("device-1")
            assertTrue(awaitItem().crowd.selectedDevices.isEmpty())
            
            viewModel.toggleDeviceSelection("device-2")
            assertTrue(awaitItem().crowd.selectedDevices.contains("device-2"))
